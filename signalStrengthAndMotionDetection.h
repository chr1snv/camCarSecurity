...
{
    if (!info || !info->buf) {
        snprintf(lastCsiInfoStr, CSI_INF_PRINT_LEN, "<%s> wifi_csi_cb\n", esp_err_to_name(ESP_ERR_INVALID_ARG));
        return;
    }
    ...
    */
    ...
    if (!s_count) {
        ...
        Serial.println("type,id,mac,rssi,rate,sig_mode,mcs,bandwidth,smoothing,not_sounding,aggregation,stbc,fec_coding,sgi,noise_floor,ampdu_cnt,channel,secondary_channel,local_timestamp,ant,sig_len,rx_state,len,first_word,data\n");
    }

    snprintf(lastCsiInfoStr, CSI_INF_PRINT_LEN, "CSI_DATA,%d," MACSTR ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
...
int staRssi;
void sense_wifi_rssi(){
  esp_wifi_sta_get_rssi(&staRssi); //get rssi of ap connected to
}